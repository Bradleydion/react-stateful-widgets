...
export default function Programmers() {
  ...
  const [feautredProgrammer, setFeaturedProgrammer]= useState()
  ...
  const getNameOfFeatured = () => {
   ...
   let pList=''
   programmersList.forEach((programmer)=> (programmer.id == feautredProgrammer)? pList = programmer.name : "")
    return pList
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (feautredProgrammer)?'gold':'royalblue', // 🤔 color turns to gold, when celebrating
  };
